...
data class HomeRow(val entry: AppEntry)

/** Pure mapping from the app snapshot plus settings to the rows on screen. */
object HomeListModel {
    const val MAX_ROWS = 16

    fun build(entries: List<AppEntry>, settings: HomeSettings): List<HomeRow> {
        if (settings.homeApps.isNotEmpty()) {
            val byKey = entries.associateBy { it.key }
            return settings.homeApps.distinct().mapNotNull { byKey[it] }.take(MAX_ROWS).map(::HomeRow)
        }
